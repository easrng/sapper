...
import Url, { URL, resolve } from 'url';
...
const build_dir = __BUILD__DIR__;
const src_dir = __SRC__DIR__;
const dev = __DEV__;
...
function URL$$1(url) {
  if (!this || this[impl] || !(this instanceof URL$$1)) {
...
Object.defineProperty(URL$$1.prototype, "href", {
  get() {
...
URL$$1.prototype.toString = function () {
  if (!this || !module.exports.is(this)) {
    throw new TypeError("Illegal invocation");
  }
  return this.href;
...
Object.defineProperty(URL$$1.prototype, "origin", {
  get() {
...
Object.defineProperty(URL$$1.prototype, "protocol", {
  get() {
...
Object.defineProperty(URL$$1.prototype, "username", {
  get() {
...
Object.defineProperty(URL$$1.prototype, "password", {
  get() {
...
Object.defineProperty(URL$$1.prototype, "host", {
  get() {
...
Object.defineProperty(URL$$1.prototype, "hostname", {
  get() {
...
Object.defineProperty(URL$$1.prototype, "port", {
  get() {
...
Object.defineProperty(URL$$1.prototype, "pathname", {
  get() {
...
Object.defineProperty(URL$$1.prototype, "search", {
  get() {
...
Object.defineProperty(URL$$1.prototype, "hash", {
  get() {
...
module.exports = {
  ...
  create(constructorArgs, privateData) {
    let obj = Object.create(URL$$1.prototype);
...
function consumeBody() {
	...
	return new Body.Promise(function (resolve$$1, reject) {
		...
		body.on('end', function () {
			...
			try {
				resolve$$1(Buffer.concat(accum, accumBytes));
			} catch (err) {
...
function get_page_handler(manifest$$1, store_getter) {
    const get_build_info = dev
        ? () => JSON.parse(readFileSync(join(build_dir, 'build.json'), 'utf-8'))
        : (assets => () => assets)(JSON.parse(readFileSync(join(build_dir, 'build.json'), 'utf-8')));
    const template = dev
        ? () => read_template(src_dir)
        : (str => () => str)(read_template(build_dir));
    const has_service_worker = existsSync(join(build_dir, 'service-worker.js'));
...
function read_template(dir = build_dir) {
    return readFileSync(`${dir}/template.html`, 'utf-8');
}
...
function middleware(opts = {}) {
    ...
    return compose_handlers([
        ...
        existsSync(join(build_dir, 'index.html')) && serve({
            pathname: '/index.html',
            cache_control: dev ? 'no-cache' : 'max-age=600'
        }),
        existsSync(join(build_dir, 'service-worker.js')) && serve({
            pathname: '/service-worker.js',
            cache_control: 'no-cache, no-store, must-revalidate'
        }),
        existsSync(join(build_dir, 'service-worker.js.map')) && serve({
...
function serve({ prefix, pathname, cache_control }) {
    ...
    const cache = new Map();
    const read = dev
        ? (file) => readFileSync(join(build_dir, file))
        : (file) => (cache.has(file) ? cache : cache.set(file, readFileSync(join(build_dir, file)))).get(file);
    return (req, res, next) => {
        if (req[IGNORE])
            return next();
        if (filter(req)) {
            ...
            try {
                const file = posix.normalize(decodeURIComponent(req.path));
