...
function patch (fs$$1) {
  ...
  if (platform === "win32") {
    fs$$1.rename = typeof fs$$1.rename !== 'function' ? fs$$1.rename
    : (function (fs$rename) {
  ...
  fs$$1.read = typeof fs$$1.read !== 'function' ? fs$$1.read
  : (function (fs$read) {
  ...
  fs$$1.readSync = typeof fs$$1.readSync !== 'function' ? fs$$1.readSync
  ...
  function patchLutimes (fs$$1) {
    if (constants.hasOwnProperty("O_SYMLINK") && fs$$1.futimes) {
...
if (!__chunk_3.fs[gracefulQueue]) {
  // This queue can be shared by multiple loaded instances
  var queue = __chunk_3.commonjsGlobal[gracefulQueue] || [];
  publishQueue(__chunk_3.fs, queue);

  // Patch fs.close/closeSync to shared queue version, because we need
  // to retry() whenever a close happens *anywhere* in the program.
  // This is essential when multiple graceful-fs instances are
  // in play at the same time.
  __chunk_3.fs.close = (function (fs$close) {
  ...
  __chunk_3.fs.closeSync = (function (fs$closeSync) {
    function closeSync (fd) {
      // This function uses the graceful-fs shared queue
      fs$closeSync.apply(__chunk_3.fs, arguments);
      resetQueue();
    }
...
if (!__chunk_3.commonjsGlobal[gracefulQueue]) {
  publishQueue(__chunk_3.commonjsGlobal, __chunk_3.fs[gracefulQueue]);
}

module.exports = patch(clone_1(__chunk_3.fs));
if (process.env.TEST_GRACEFUL_FS_GLOBAL_PATCH && !__chunk_3.fs.__patched) {
    module.exports = patch(__chunk_3.fs);
    __chunk_3.fs.__patched = true;
}
...
function patch (fs$$1) {
  ...
  function readFile (path$$1, options, cb) {
    ...
    function go$readFile (path$$1, options, cb, startTime) {
      return fs$readFile(path$$1, options, function (err) {
        if (err && (err.code === 'EMFILE' || err.code === 'ENFILE'))
          enqueue([go$readFile, [path$$1, options, cb], err, startTime || Date.now(), Date.now()]);
        else {
          if (typeof cb === 'function')
            cb.apply(this, arguments);
        }
  ...
  function writeFile (path$$1, data, options, cb) {
    ...
    function go$writeFile (path$$1, data, options, cb, startTime) {
      return fs$writeFile(path$$1, data, options, function (err) {
        if (err && (err.code === 'EMFILE' || err.code === 'ENFILE'))
          enqueue([go$writeFile, [path$$1, data, options, cb], err, startTime || Date.now(), Date.now()]);
        else {
          if (typeof cb === 'function')
            cb.apply(this, arguments);
        }
  ...
  function appendFile (path$$1, data, options, cb) {
    ...
    function go$appendFile (path$$1, data, options, cb, startTime) {
      return fs$appendFile(path$$1, data, options, function (err) {
        if (err && (err.code === 'EMFILE' || err.code === 'ENFILE'))
          enqueue([go$appendFile, [path$$1, data, options, cb], err, startTime || Date.now(), Date.now()]);
        else {
          if (typeof cb === 'function')
            cb.apply(this, arguments);
        }
  ...
  var fs$copyFile = fs$$1.copyFile;
  if (fs$copyFile)
    fs$$1.copyFile = copyFile;
  ...
  function readdir (path$$1, options, cb) {
    ...
    var go$readdir = noReaddirOptionVersions.test(process.version)
      ? function go$readdir (path$$1, options, cb, startTime) {
        return fs$readdir(path$$1, fs$readdirCallback(
          path$$1, options, cb, startTime
        ))
      }
      : function go$readdir (path$$1, options, cb, startTime) {
        return fs$readdir(path$$1, options, fs$readdirCallback(
          path$$1, options, cb, startTime
        ))
      };

    return go$readdir(path$$1, options, cb)

    function fs$readdirCallback (path$$1, options, cb, startTime) {
  ...
  function open (path$$1, flags, mode, cb) {
    ...
    function go$open (path$$1, flags, mode, cb, startTime) {
      return fs$open(path$$1, flags, mode, function (err, fd) {
        if (err && (err.code === 'EMFILE' || err.code === 'ENFILE'))
          enqueue([go$open, [path$$1, flags, mode, cb], err, startTime || Date.now(), Date.now()]);
        else {
          if (typeof cb === 'function')
            cb.apply(this, arguments);
        }
...
function enqueue (elem) {
  ...
  __chunk_3.fs[gracefulQueue].push(elem);
  retry();
}
...
function resetQueue () {
  var now = Date.now();
  for (var i = 0; i < __chunk_3.fs[gracefulQueue].length; ++i) {
    // entries that are only a length of 2 are from an older version, don't
    // bother modifying those since they'll be retried anyway.
    if (__chunk_3.fs[gracefulQueue][i].length > 2) {
      __chunk_3.fs[gracefulQueue][i][3] = now; // startTime
      __chunk_3.fs[gracefulQueue][i][4] = now; // lastTime
    }
  }
...
function retry () {
  ...
  if (__chunk_3.fs[gracefulQueue].length === 0)
    return

  var elem = __chunk_3.fs[gracefulQueue].shift();
  var fn = elem[0];
  var args = elem[1];
...
var utils = __chunk_3.createCommonjsModule(function (module) {
...
var utils_1 = utils.mixin;
var utils_2 = utils.wrapperSymbol;
var utils_3 = utils.implSymbol;
var utils_4 = utils.wrapperForImpl;
var utils_5 = utils.implForWrapper;
...
var mappingTable$2 = __chunk_3.getCjsExportFromNamespace(mappingTable$1);
...
function findStatus(val) {
  var start = 0;
  var end = mappingTable$2.length - 1;

  while (start <= end) {
    var mid = Math.floor((start + end) / 2);

    var target = mappingTable$2[mid];
...
var urlStateMachine = __chunk_3.createCommonjsModule(function (module) {
...
var URL_1 = __chunk_3.createCommonjsModule(function (module) {
...
var URL_2 = URL_1.is;
var URL_3 = URL_1.create;
var URL_4 = URL_1.setup;
var URL_5 = URL_1.expose;

var URL = URL_1.interface;
...
const STATUS_CODES = http__default.STATUS_CODES;
...
const URL$1 = Url__default.URL || publicApi.URL;
...
const URL$1$1 = Url__default.URL || publicApi.URL;
                ...
                return tslib.__generator(this, function (_a) {
                    switch (_a.label) {
                        ...
                        case 2:
                            ...
                            base = resolve(url.href, base_href);
                            match = void 0;
                            pattern = /<a ([\s\S]+?)>/gm;
                            _loop_1 = function () {
                                var attrs = match[1];
